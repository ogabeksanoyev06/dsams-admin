...
const { deleteBlogById, getBlogs } = blogStore
...
const confirmDelete = async () => {
  try {
    loading.value = true
    const res = await deleteBlogById(props.blogId)
    if (res.status) {
      ...
      <DialogContent class="sm:max-w-[600px] p-4">
        <div class="pt-6 p-5 flex flex-col items-center text-center">
          ...
          <div class="flex gap-2 w-full mt-7">
            ...
            <Button :loading="loading" variant="destructive" size="lg" class="w-full" @click="confirmDelete"> Удалить </Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
